...
Therefore, instead of using `oa_fetch()`, you can use `oa_query()` and `oa_request()` separately to build your own query, then (optionally) convert the result to a dataframe using `oa2df()`.
...
You will need to first build your own query (*e.g.*, using `oa_query()`) to use as an argument in `oa_generate()`.
